...
echo ========================================
echo MSFT Historical Analysis - May & June 2025
echo ========================================
echo.

REM Clear MSFT cache first
echo Clearing MSFT cache...
python -c "from tradingagents.dataflows.interface import clear_cache_data; result = clear_cache_data(symbol='MSFT'); print(f'Cache cleared: {result}')"
echo.
...
REM Set log file
set LOGFILE=logs\msft_historical_%DATE:~-4,4%%DATE:~-10,2%%DATE:~-7,2%_%TIME:~0,2%%TIME:~3,2%.log
...
for %%d in (%MAY_DAYS%) do (
    echo.
    echo [%TIME%] Starting MSFT analysis for %%d...
    echo [%TIME%] Starting MSFT analysis for %%d... >> "%LOGFILE%"
    
    python main.py MSFT %%d
    if !ERRORLEVEL! EQU 0 (
        echo [%TIME%] SUCCESS: MSFT analysis completed for %%d
        echo [%TIME%] SUCCESS: MSFT analysis completed for %%d >> "%LOGFILE%"
    ) else (
        echo [%TIME%] FAILED: MSFT analysis failed for %%d
        echo [%TIME%] FAILED: MSFT analysis failed for %%d >> "%LOGFILE%"
    )
...
echo.
echo ========================================
echo MSFT Historical Analysis Complete
echo ========================================
